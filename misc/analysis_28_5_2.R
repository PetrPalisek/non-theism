...
summary(base_mi, output = "data.frame", std = "std.all")
base_mi_summary <- summary(base_mi, output = "data.frame", std = "std.all")
coef(base_mi)
vcov(base_mi)
...
indices <- c(which(base_mi_summary[, 'lhs'] == "h1a_"),
             which(base_mi_summary[, 'lhs'] == "h1b_"),
             which(base_mi_summary[, 'lhs'] == "h1c_"),
             which(base_mi_summary[, 'lhs'] == "ms1_big3"),
             which(base_mi_summary[, 'lhs'] == "ms1_big4"))


est <- standardizedSolution(base_mi)[indices, 'est.std'] # estimates
...
controls_mi <- semTools::runMI(controls, mice.imp, fun = "sem", ordered = c("BiG1","BiG2", "BiG3", "BiG4", "ParRit"), meanstructure = T,
                           estimator = "WLSMV", missing = "pairwise", parameterization = "theta")
